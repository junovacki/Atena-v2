...
<body id="body">
    ...
    <form id="formCadastro">
        <div id="campoLogin">
            Nome da turma: <input type="text" name="nome_turma" id="nome_turma"/>
        </div>
        <div id="campoLogin">
            Serie da turma: <input type="text" name="serie_turma" id="serie_turma"/>
        </div>
        <div id="campoLogin">
            Ano da turma: <input type="text" name="ano_turma" id="ano_turma"/>
        </div>
        <div id="campoLogin2">
            Semestre da turma: <input type="text" name="semestre_turma" id="semestre_turma"/>
        </div>
        <div id="campoTipoUsuario">
            Grade da turma: <select id="tipoCurso" name="tipoCurso" style="margin-left: 14px;">
            @foreach($grades as $grade)
                <option value="<?= $grade->id?>"><?= $grade->descricao?></option>
            @endforeach
            </select>
        </div>
...
function enviar() {
    ...
    let eTurno = document.getElementById('tipoCurso');
    let turno= eTurno.options[eTurno.selectedIndex].value;

        axios.post('http://127.0.0.1:8000/api/registrarTurma', {
