...
if($_COOKIE['nivelAcesso'] == 2){
    ...
    $prova_perguntas = DB::select("SELECT * FROM prova_perguntas
                        LEFT JOIN perguntas ON prova_perguntas.idPergunta=perguntas.id
                        where prova_perguntas.idProva = ?", [$id]);
    $provas = DB::select("SELECT * FROM provas where id = ?", [$id]);
    $turma_provas = DB::select("SELECT * FROM turma_provas where idProva = ?", [$id]);
    foreach ($prova_perguntas as $pp) {
        foreach ($perguntas as $key => $pergunta) {
            if($pergunta->id == $pp->idDisciplina ){
                $pergunta->utilizado = 'active';
                $pergunta->check = true;
            }else{
                $pergunta->utilizado = '';
...
<body id="body">
    ...
    <form id="formCadastro">
        ...
        <div id="campoLogin">
            Descrição da prova: <input type="text" name="nome_prova" id="nome_prova" value="<?= $provas[0]->descricao?>"/>
        </div>
        <div id="campoLogin">
            Data da prova: <input type="text" name="nome_prova" id="data_prova" value="<?= $provas[0]->dataProva?>"/>
        </div>
        Turma: <select id="curso" name="curso" style="margin-left: 0px; max-width: 100px;">
            @foreach($turmas as $turma)
                <option value="<?= $turma->id?>" <?php if($turma_provas[0]->idTurma == $turma->id){echo "selected";}?>><?= $turma->descricao?></option>
            @endforeach
            </select>
                      ...
                      <div class="table-responsive">
                        ...
                        <table class="table custom-table" id="myTable">
                          ...
                          <tbody>
                            ...
                            <tr class="<?= $pergunta->idDisciplina ?>  <?= $pergunta->utilizado ?>">
                                <th scope="row" >
                                    <label class="control control--checkbox">
                                        @if ($pergunta->check == true)
                                            <input type="checkbox" checked value="<?= $pergunta->id ?>" />

                                        @else
                                            <input type="checkbox" value="<?= $pergunta->id ?>" />
...
function enviar() {
    ...
    console.log(final);
        ...
        axios.post('http://127.0.0.1:8000/api/atualizarProva', {
