...
use Illuminate\Support\Facades\DB;
    if($_COOKIE['nivelAcesso'] == 1){
        $cursos = DB::select("SELECT cursos.id, cursos.nome_curso, cursos.ativo, cursos.idModalidade, cursos.idTurno, cursos.idCategoria, modalidades.modalidade, categorias.categoria, turnos.turno
                    FROM cursos
                    LEFT JOIN modalidades
                    ON cursos.idModalidade = modalidades.id
                    LEFT JOIN categorias
                    ON cursos.idCategoria = categorias.id
                    LEFT JOIN turnos
                    ON cursos.idTurno = turnos.id
                    ");
    }
?>
<!DOCTYPE html>
...
<body id="body">
                                    ...
                                    <table class="table table-hover">
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                @if ($curso->ativo == '1')
                                                    <a  onclick="desativarCurso('<?= $curso->id?>')"  >
                                                        <i class="fa fa-toggle-on" aria-hidden="true" style="font-size: 30px"></i>
                                                    </a>
                                                @else
                                                    <a   onclick="ativarCurso('<?= $curso->id?>')" >
                                                ...
                                                <td>
                                                    ...
                                                    <a  class="button" id="view"  onclick="deletarCurso('<?= $curso->id?>')">
    ...
    <script>
        ...
        if (confirm(text) == true) {
            axios.post('http://127.0.0.1:8000/api/deletarCurso', {
            id: id
            })
        ...
        let text = "Deseja realmente desativar o curso?";
        if (confirm(text) == true) {
            axios.post('http://127.0.0.1:8000/api/desativar-curso', {
            id: id
            })
            .then(function (response) {
        ...
        function ativarCurso(id) {
        let text = "Deseja realmente ativar o curso?";
        if (confirm(text) == true) {
            axios.post('http://127.0.0.1:8000/api/ativar-curso', {
            id: id
            })
            .then(function (response) {
